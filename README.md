...
本项目提供了在线Excel模板设计器，支持通过Web界面可视化设计Excel模板。

### 访问方式

启动服务后访问：http://localhost:8222/excel-service/designer

### 功能特性

- ✅ 在线Excel编辑（基于Luckysheet）
- ✅ 完整的Excel功能支持（公式、样式、图表等）
- ✅ 模板文件上传、下载、管理
- ✅ 占位符自动识别与管理
- ✅ 自定义字体上传与管理
- ✅ Excel文件导入导出
...
点击「新建模板」按钮，输入模板名称后即可创建空白模板。

#### 2. 设计模板

- 使用工具栏设置单元格样式、字体、边框等
- 支持Excel公式（如 `=SUM(A1:A10)`）
- 支持插入图表
- 在单元格中输入 `{fieldName}` 作为数据占位符
...
| 占位符类型 | 格式示例 | 说明 |
|-----------|---------|------|
| 数据行占位符 | `{userName}`、`{age}` | 用于填充数据行的字段值 |
| 文档级占位符 | `{time}`、`{date}`、`{year}` | 用于填充文档标题或汇总信息 |

**占位符使用示例**：

```excel
┌─────────────────────────────────────────┐
│          员工统计报表 - {year}年         │  <- 文档级占位符
├──────┬──────┬──────┬──────────────────┤
│ 姓名  │ 年龄  │ 部门  │ 工资              │  <- 表头
├──────┼──────┼──────┼──────────────────┤
│ {userName} │ {age} │ {dept} │ {salary} │  <- 数据行占位符
└──────┴──────┴──────┴──────────────────┘
```

#### 4. 保存模板

设计完成后点击「保存模板」按钮，模板将保存到配置的模板目录中，可直接用于数据导出。

#### 5. 导入现有Excel

支持导入已有的Excel文件作为模板基础，点击「导入Excel」选择文件即可。

---

## 字体管理

模板设计器支持上传自定义字体，上传后的字体可在所有模板中使用。

### 字体上传

1. 在模板设计器左侧「自定义字体」面板中
2. 点击上传区域或拖拽字体文件到上传区
3. 支持格式：`.ttf`、`.otf`、`.woff`、`.woff2`

### 字体存储

- 字体文件存储在服务器指定目录（默认：`${user.home}/excel-service/fonts`）
- 上传后的字体所有用户共享使用
- 页面刷新后自动加载已上传的字体

### 字体配置

在 `application.yml` 中配置字体存储路径：

```yaml
excel:
  font:
    directory: ${user.home}/excel-service/fonts  # 字体存储目录
    allowed-extensions:
      - .ttf
      - .otf
      - .woff
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/designer/api/fonts/list` | GET | 获取字体列表 |
| `/designer/api/fonts/upload` | POST | 上传字体文件 |
| `/designer/api/fonts/download/{fontName}` | GET | 下载字体文件 |
| `/designer/api/fonts/delete/{fontName}` | DELETE | 删除字体文件 |
| `/designer/api/fonts/css-all` | GET | 获取所有字体CSS |
...
excel:
  template:
    default-dir: /home/excel-templates  # 模板存储目录
    allowed-extensions:
      - .xlsx
      - .xls
```
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/template/api/templates` | GET | 获取模板列表 |
| `/template/api/upload` | POST | 上传模板文件 |
| `/template/api/download/{fileName}` | GET | 下载模板文件 |
| `/template/api/delete/{fileName}` | DELETE | 删除模板文件 |

### 模板管理页面

访问 http://localhost:8222/excel-service/template 可进入模板管理页面，支持：
- 查看所有模板列表
- 上传新模板
- 下载模板
- 删除模板
